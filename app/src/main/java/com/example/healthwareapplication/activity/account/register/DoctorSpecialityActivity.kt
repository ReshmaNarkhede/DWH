...
import com.example.healthwareapplication.databinding.ActivityDoctorSpecialityBinding
...
class DoctorSpecialityActivity : AppCompatActivity() {
    private lateinit var binding: ActivityDoctorSpecialityBinding
    private var specArray: JSONArray? = JSONArray()
    private lateinit var userDetailModel: UserDetailModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityDoctorSpecialityBinding.inflate(layoutInflater)
    ...
    private fun defaultConfiguration() {
        ...
        binding.specialityEdtTxt.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence, i: Int, i1: Int, i2: Int) {
                if(s.toString().isNotEmpty()) {
                    binding.specialityList.visibility = View.VISIBLE
                    binding.specLayout.background = resources.getDrawable(R.drawable.country_btn_selector)
            ...
            override fun afterTextChanged(editable: Editable) {
                if(editable.toString().isEmpty()){
                    binding.specialityList.visibility = View.GONE
                    binding.specLayout.background = resources.getDrawable(R.drawable.btn_selector)
                }
            }
        })
    }
    ...
    private fun checkValidation() {
        var isFlag = true
        if (binding.specialityEdtTxt.text!!.trim().isEmpty()) {
            AppHelper.showToast(this, getString(R.string.valid_speciality))
            isFlag = false
        }
        if (binding.experienceEdtTxt.text!!.trim().isEmpty()) {
    ...
    private fun goToNext() {
        val speciality = binding.specialityEdtTxt.text.toString()
        val experience = binding.experienceEdtTxt.text.toString()
